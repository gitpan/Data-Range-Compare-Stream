...
Given multiple sets of 2d data, this package will organize consolidate and explain gaps and intersections in the given sets of data.  

=head2 OverView

The objective of this overview is to compare 3 sets of ranges side by side: see  Figure 1
...
When we look closely at the data ( Figure 1 ) we see "Numeric Range Set: A" contains ranges that overlap with each other.  When trying to align each set of data against another it's not hard to tell that range sets A B and C have almost no common boundary points, only overlap to some degree, and the sets of data are not contiguous.


Part of the process of comparing sets of data involves making sure there are no duplicates or overlaps in each individual source of data.  The first action taken is the removal of duplicate and consolidation of overlapping ranges, this process can be seen by looking at the resulting confversion for "Numeric Range set: A" seen in  see: Figure 2.
...
The Data::Range::Compare::Stream library solves all of the above problems, allowing for a clean and simple side by side comparison of each set of data in parallel.  We arn't just conserned about what ranges overlap, but also at what boundry range(s) that do overlap:  see figure 4.
...
The final stage, only ranges dirived from our original source data is presented.  In reality this layred process happens almost completly in parallel, and can easly be abstracted to fit almost any data set type.
