...
=item * my $iterator=new Data::Range::Compare::Stream::Iterator::Array;
  ...
  Instance Constructor, all arguments are optional
	...
	Defines the array refernce used internally
...
=item * $iterator->add_range($obj)
  ...
  Adds $obj to the internal array.

    This function returns false once $iterator has been sorted

=item * $iterator->create_range(0,0);

  Creates a new range object and adds it to the internal array.

    This function returns false once $iterator has been sorted

=item * $iterator->prepare_for_consolidate_asc;

  Sorts the internal array and sets the sorted flag

    $iterator->has_next returns false until the internal array has been sorted
