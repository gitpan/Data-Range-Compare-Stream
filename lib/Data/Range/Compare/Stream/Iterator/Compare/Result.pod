...
=item * my $result=new Data::Range::Compare::Stream::Iterator::Compare::Result (COMMON_RANGE,[OVERLAP_RESULTS],[OVERLAP_IDS],[NON_OVERLAP_IDS]);

  Objet constructor:

    All argumetns are mandiotry
    ...
    NON_OVERLAP_IDS
      ...
      An array ref represetnig the ids of OVERLAP_RESULTS that are undef
...
=item * my $range=$result->get_consolidator_result_by_id($id);

  $id represents the column

  $range will be undef if the column did not overlap with the common range
  
    or

  $range will be an object that implements Daa::Range::Compare::Stream

=item * my $range=$result->get_result_by_id($id);

  alias to get_consolidator_result_by_id

=item * my $boolean=$result->is_empty;

  $boolean is true when no ranges overlaped with the common range
...
=item * my $ref=$result->get_non_overlapping_containers;
  ...
  Returns an array ref of undefs for every column that did not overlap
