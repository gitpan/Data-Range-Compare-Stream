...
Comparing DateTime ranges requires having 2 DateTime objects per range: One as the start value
and one as the end value.  This means the constructor call will get a little ugly, becase the new 
involves 2 DateTime->new(xxx) calls and all of thier arguments for the new class.  On the positive
side the interface implementation is pretty straight forward.
